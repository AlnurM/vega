How It Works
(Here's a complete GitHub repo for you to download and try.)
A workflow that:
1. Cursor agent reads prompt.md instructions
2. Agent picks the next story from prd.json
3. Agent implements it
4. Agent runs typecheck + tests
5. Agent commits if passing
6. Agent marks story done
7. Agent logs learnings
8. Process repeats until done

Memory persists only through:
- Git commits
- progress.txt (learnings)
- prd.json (task status)
- .cursorrules (codebase patterns)

File Structure
```
project-root/
├── .cursorrules          # Project-wide patterns for Cursor agents
└── scripts/ralph/
    ├── prompt.md         # Instructions for each iteration
    ├── prd.json          # Task list with user stories
    └── progress.txt      # Session memory and learnings
```

## .cursorrules Setup

Create `.cursorrules` at project root to help Cursor agents understand your codebase:

```markdown
# Codebase Patterns

## Database
- Migrations: Always use IF NOT EXISTS
- Schema changes require updating server actions

## React/TypeScript
- Refs: useRef<Timeout | null>(null)
- Components: Export from index.ts

## Testing
- Run typecheck before committing: npm run typecheck
- UI tests require dev server on port 3000
```

This file is read by Cursor agents automatically and helps maintain consistency.

## Using Cursor Agents

Cursor agents work best when:
1. You provide clear instructions in `.cursorrules` or `prompt.md`
2. You open the conversation with the prompt from `prompt.md`
3. The agent has access to `prd.json` and `progress.txt`
4. You let the agent work through stories iteratively

## Manual Iteration Workflow

Since Cursor doesn't have a CLI loop, you can:

**Option 1: Manual iteration**
- Open Cursor
- Paste contents of `prompt.md` into chat
- Let agent complete one story
- Repeat until all stories pass

**Option 2: Automated via script**
Create `scripts/ralph/ralph.sh`:
```bash
#!/bin/bash
set -e

MAX_ITERATIONS=${1:-10}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "🚀 Starting Ralph workflow"
echo "Open Cursor and paste the following prompt:"
echo ""
cat "$SCRIPT_DIR/prompt.md"
echo ""
echo "After each iteration, check if all stories pass:"
echo "cat $SCRIPT_DIR/prd.json | jq '.userStories[] | {id, passes}'"
echo ""
echo "When all pass, the agent should reply: <promise>COMPLETE</promise>"
```

Make executable:
```bash
chmod +x scripts/ralph/ralph.sh
```
prompt.md
Instructions for each iteration:
```markdown
# Ralph Agent Instructions

## Your Task

1. Read `scripts/ralph/prd.json`
2. Read `scripts/ralph/progress.txt`
   (check Codebase Patterns first)
3. Read `.cursorrules` if it exists (project root)
4. Check you're on the correct branch
5. Pick highest priority story 
   where `passes: false`
6. Implement that ONE story
7. Run typecheck and tests
8. Update AGENTS.md files with learnings (in relevant directories)
9. Update `.cursorrules` if you discovered project-wide patterns
10. Commit: `feat: [ID] - [Title]`
11. Update prd.json: `passes: true`
12. Append learnings to progress.txt

## Progress Format

APPEND to progress.txt:

## [Date] - [Story ID]
- What was implemented
- Files changed
- **Learnings:**
  - Patterns discovered
  - Gotchas encountered
---

## Codebase Patterns

Add reusable patterns to the TOP 
of progress.txt:

## Codebase Patterns
- Migrations: Use IF NOT EXISTS
- React: useRef<Timeout | null>(null)

Also update `.cursorrules` at project root for project-wide patterns.

## Stop Condition

If ALL stories pass, reply:
<promise>COMPLETE</promise>

Otherwise end normally and indicate which story was completed.
```
prd.json
Your task list:
json
{
  "branchName": "ralph/feature",
  "userStories": [
    {
      "id": "US-001",
      "title": "Add login form",
      "acceptanceCriteria": [
        "Email/password fields",
        "Validates email format",
        "typecheck passes"
      ],
      "priority": 1,
      "passes": false,
      "notes": ""
    }
  ]
}
Key fields:
`branchName` — branch to use
`priority` — lower = first
`passes` — set true when done
progress.txt
Start with context:
markdown
# Ralph Progress Log
Started: 2024-01-15

## Codebase Patterns
- Migrations: IF NOT EXISTS
- Types: Export from actions.ts

## Key Files
- db/schema.ts
- app/auth/actions.ts
---
Ralph appends after each story.
Patterns accumulate across iterations.
Running Ralph in Cursor

**Step 1: Setup**
```bash
# Ensure you have the ralph files
ls scripts/ralph/
# Should show: prompt.md, prd.json, progress.txt
```

**Step 2: Start Cursor Agent**
1. Open Cursor in your project
2. Open chat (Cmd+L / Ctrl+L)
3. Paste the contents of `scripts/ralph/prompt.md`
4. The agent will:
   - Read `prd.json` and `progress.txt`
   - Check current branch
   - Pick highest priority story where `passes: false`
   - Implement it
   - Run typecheck and tests
   - Commit if passing
   - Update `prd.json` and `progress.txt`
   - Continue to next story

**Step 3: Monitor Progress**
```bash
# Check story status
cat scripts/ralph/prd.json | jq '.userStories[] | {id, passes}'

# View learnings
cat scripts/ralph/progress.txt

# View commits
git log --oneline -10
```

**Step 4: Continue Iterations**
- If not all stories pass, paste `prompt.md` again in a new chat
- Agent will pick up where it left off
- Repeat until agent replies: `<promise>COMPLETE</promise>`

Ralph will:
- Create the feature branch (if needed)
- Complete stories one by one
- Commit after each passing story
- Stop when all pass
Critical Success Factors
1. Small Stories
Must fit in one context window.
plaintext
❌ Too big:
> "Build entire auth system"
✅ Right size:
> "Add login form"
> "Add email validation"
> "Add auth server action"
2. Feedback Loops
Ralph needs fast feedback:
`npm run typecheck`
`npm test`
Without these, broken code compounds.
3. Explicit Criteria
plaintext
❌ Vague:
> "Users can log in"
✅ Explicit:
> - Email/password fields
> - Validates email format
> - Shows error on failure
> - typecheck passes
> - Verify at localhost:$PORT/login (PORT defaults to 3000)
4. Learnings Compound
By story 10, Ralph knows patterns from stories 1-9.
Three places for learnings:
- progress.txt — session memory for Ralph iterations
- AGENTS.md — permanent docs for humans and future agents (in relevant directories)
- .cursorrules — project-wide patterns for Cursor agents (at root)

Before committing, Ralph updates:
- AGENTS.md files in directories with edited files (if it discovered reusable patterns)
- .cursorrules at project root (for project-wide patterns)

5. AGENTS.md and .cursorrules Updates
Ralph updates these when it learns something worth preserving:

✅ Good additions:
- "When modifying X, also update Y"
- "This module uses pattern Z"
- "Tests require dev server running"
- "Migrations must use IF NOT EXISTS"
- "React refs: useRef<Timeout | null>(null)"

❌ Don't add:
- Story-specific details
- Temporary notes
- Info already in progress.txt

.cursorrules format (at project root):
```
# Codebase Patterns

## Database
- Migrations: Always use IF NOT EXISTS
- Schema changes require updating server actions

## React
- Refs: useRef<Timeout | null>(null)
- Components: Export from index.ts

## Testing
- Run typecheck before committing
- UI tests require dev server on port 3000
```
6. Browser Testing
For UI changes, verify in browser:
- Start dev server: `npm run dev` (or your project's command)
- Navigate to the page in browser
- Take screenshot or verify visually
- For automated testing, use Playwright or similar:
```bash
# Install Playwright if needed
npx playwright install

# Create test script
npx playwright test --headed
```

Or use Cursor's built-in browser tools if available.
Not complete until verified visually or with screenshot.
Common Gotchas
Idempotent migrations:
sql
ADD COLUMN IF NOT EXISTS email TEXT;
Interactive prompts:
bash
echo -e "\n\n\n" | npm run db:generate
Schema changes:
After editing schema, check:
Server actions
UI components
API routes
Fixing related files is OK:
If typecheck requires other changes, make them. Not scope creep.
Monitoring
bash
# Story status
cat scripts/ralph/prd.json | \
jq '.userStories[] | {id, passes}'
# Learnings
cat scripts/ralph/progress.txt
# Commits
git log --oneline -10
## Cursor Agent Capabilities

Cursor agents can:
- Read and write files directly
- Run terminal commands (typecheck, tests)
- Make git commits
- Understand codebase context through codebase search
- Follow instructions from `.cursorrules` and `prompt.md`

## Tips for Cursor Agents

1. **Clear Instructions**: Be explicit in `prompt.md` about what to do
2. **Context**: Agent automatically has access to open files and codebase
3. **Iterative Work**: Let agent complete one story before moving to next
4. **Verification**: Always verify typecheck and tests pass before committing
5. **Patterns**: Update `.cursorrules` as you discover reusable patterns

Real Results
We built an evaluation system:
13 user stories
~15 iterations
2-5 min each
~1 hour total
Learnings compound. By story 10, Ralph knew our patterns.

With Cursor agents, you get:
- Better codebase understanding through semantic search
- Automatic context from open files
- Built-in code editing capabilities
- Seamless git integration